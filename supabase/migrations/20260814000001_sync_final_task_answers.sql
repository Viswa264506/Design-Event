-- Migration: Synchronize task_answers and evaluate_submission_rpc with 10 Final Questions

-- 1. Truncate stale configuration
truncate table public.task_answers cascade;
truncate table public.tasks cascade;

-- 2. Populate public.tasks
...
-- 3. Populate public.task_answers
...
('task_03', '{
  ...
  "elements": [
    { "id": "button_base", "type": "rectangle", "x": 50, "y": 400, "width": 100, "height": 30, "color": "#2563EB", "backgroundColor": "#2563EB", "borderRadius": 0, "name": "Button Base" },
...
-- 4. Create or replace atomic evaluation RPC stored procedure in PostgreSQL
create or replace function public.evaluate_submission_rpc(
  p_design_json jsonb,
  p_session_id text
)
returns jsonb
...
declare
  ...
  v_task record;
  v_expected_elements jsonb;
  v_initial_elements jsonb;
  v_student_elements jsonb;
  v_total_score numeric := 0;
  v_task_score numeric := 0;
  v_is_modified boolean := false;
  v_submission_id uuid;
  v_submitted_at text := to_char(now() at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"');
  v_task_details jsonb := '[]'::jsonb;
  v_feedback text;
  v_c1 numeric := 0; v_c2 numeric := 0; v_c3 numeric := 0; v_c4 numeric := 0; v_c5 numeric := 0;
  v_el1 jsonb; v_el2 jsonb; v_el3 jsonb; v_el4 jsonb; v_el5 jsonb; v_el6 jsonb;
  v_stu_col text; v_diff_x numeric; v_diff_y numeric; v_diff_w numeric; v_diff_h numeric;
begin
  -- Look up participant by active session ID lock
  ...
  if v_participant.id is null then
    return jsonb_build_object('success', false, 'error', 'Participant session expired or lock not found');
  end if;

  if v_participant.status = 'submitted' then
    return jsonb_build_object('success', false, 'error', 'Submission locked. Challenge already finalized.');
  end if;

  -- Iterate through task answers
  for v_task in select * from public.task_answers loop
    v_expected_elements := coalesce(v_task.target_layout->'elements', '[]'::jsonb);
    v_initial_elements := coalesce(p_design_json->'initialDesigns'->v_task.task_id, v_task.initial_layout->'elements', '[]'::jsonb);
    v_student_elements := coalesce(p_design_json->'designs'->v_task.task_id, p_design_json->v_task.task_id, '[]'::jsonb);
    
    -- Attempt detection: Compare student submitted JSON against initial layout JSON
    v_is_modified := (v_student_elements::text <> v_initial_elements::text) or (jsonb_array_length(v_student_elements) <> jsonb_array_length(v_initial_elements));

    if not v_is_modified then
      v_task_score := 0;
      v_feedback := 'Not attempted.';
    else
      v_c1 := 0; v_c2 := 0; v_c3 := 0; v_c4 := 0; v_c5 := 0;

      case v_task.task_id
        when 'task_01' then
          select value into v_el1 from jsonb_array_elements(v_student_elements) where value->>'id' = 'logo_element' limit 1;
          if v_el1 is not null and v_el1->>'type' = 'image' then v_c1 := 2.0; end if;
          if v_el1 is not null then
            v_diff_x := abs(coalesce((v_el1->>'x')::numeric, 0) - 690);
            v_diff_y := abs(coalesce((v_el1->>'y')::numeric, 0) - 30);
            if v_diff_x <= 3 and v_diff_y <= 3 then v_c2 := 2.0;
            elsif v_diff_x <= 10 and v_diff_y <= 10 then v_c2 := 1.0; end if;

            v_diff_w := abs(coalesce((v_el1->>'width')::numeric, 0) - 80);
            v_diff_h := abs(coalesce((v_el1->>'height')::numeric, 0) - 80);
            if v_diff_w <= 3 and v_diff_h <= 3 then v_c3 := 2.0;
            elsif v_diff_w <= 10 and v_diff_h <= 10 then v_c3 := 1.0; end if;

            if coalesce((v_el1->>'height')::numeric, 0) > 0 then
              if abs((coalesce((v_el1->>'width')::numeric, 0) / (v_el1->>'height')::numeric) - 1.0) <= 0.05 then v_c4 := 2.0;
              elsif abs((coalesce((v_el1->>'width')::numeric, 0) / (v_el1->>'height')::numeric) - 1.0) <= 0.2 then v_c4 := 1.0; end if;
            end if;

            if abs((800 - coalesce((v_el1->>'x')::numeric, 0) - coalesce((v_el1->>'width')::numeric, 0)) - 30) <= 3 and abs(coalesce((v_el1->>'y')::numeric, 0) - 30) <= 3 then v_c5 := 2.0;
            elsif abs((800 - coalesce((v_el1->>'x')::numeric, 0) - coalesce((v_el1->>'width')::numeric, 0)) - 30) <= 10 then v_c5 := 1.0; end if;
          end if;

        when 'task_02' then
          select value into v_el1 from jsonb_array_elements(v_student_elements) where value->>'id' = 'heading_text' limit 1;
          if v_el1 is not null then
            if lower(trim(coalesce(v_el1->>'text', ''))) = 'design with purpose' then v_c1 := 2.0;
            elsif position('design with purpose' in lower(trim(coalesce(v_el1->>'text', '')))) > 0 then v_c1 := 1.0; end if;

            if position('inter' in lower(coalesce(v_el1->>'fontFamily', 'inter'))) > 0 then v_c2 := 2.0; end if;

            if abs(coalesce((v_el1->>'fontSize')::numeric, 0) - 32) <= 2 then v_c3 := 2.0;
            elsif abs(coalesce((v_el1->>'fontSize')::numeric, 0) - 32) <= 6 then v_c3 := 1.0; end if;

            if coalesce((v_el1->>'fontWeight')::numeric, 0) = 700 then v_c4 := 2.0;
            elsif coalesce((v_el1->>'fontWeight')::numeric, 0) >= 600 then v_c4 := 1.0; end if;

            if coalesce(v_el1->>'align', 'left') = 'center' and abs(coalesce((v_el1->>'x')::numeric, 0) - 200) <= 10 then v_c5 := 2.0;
            elsif coalesce(v_el1->>'align', 'left') = 'center' or abs(coalesce((v_el1->>'x')::numeric, 0) - 200) <= 10 then v_c5 := 1.0; end if;
          end if;

        when 'task_03' then
          select value into v_el1 from jsonb_array_elements(v_student_elements) where value->>'id' = 'button_base' limit 1;
          select value into v_el2 from jsonb_array_elements(v_student_elements) where value->>'id' = 'button_text' limit 1;
          if v_el1 is not null and v_el2 is not null then v_c1 := 2.0;
          elsif v_el1 is not null or v_el2 is not null then v_c1 := 1.0; end if;

          if v_el1 is not null then
            if abs(coalesce((v_el1->>'x')::numeric, 0) - 310) <= 5 and abs(coalesce((v_el1->>'y')::numeric, 0) - 270) <= 5 then v_c2 := 2.0;
            elsif abs(coalesce((v_el1->>'x')::numeric, 0) - 310) <= 15 then v_c2 := 1.0; end if;

            if abs(coalesce((v_el1->>'width')::numeric, 0) - 180) <= 5 and abs(coalesce((v_el1->>'height')::numeric, 0) - 50) <= 5 then v_c3 := 2.0;
            elsif abs(coalesce((v_el1->>'width')::numeric, 0) - 180) <= 15 then v_c3 := 1.0; end if;

            v_stu_col := upper(coalesce(v_el1->>'color', v_el1->>'backgroundColor', ''));
            if v_stu_col = '#2563EB' and abs(coalesce((v_el1->>'borderRadius')::numeric, 0) - 10) <= 2 then v_c4 := 2.0;
            elsif v_stu_col = '#2563EB' or abs(coalesce((v_el1->>'borderRadius')::numeric, 0) - 10) <= 2 then v_c4 := 1.0; end if;
          end if;

          if v_el2 is not null then
            if upper(trim(coalesce(v_el2->>'text', ''))) = 'REGISTER NOW' then v_c5 := 2.0;
            elsif position('REGISTER' in upper(trim(coalesce(v_el2->>'text', '')))) > 0 then v_c5 := 1.0; end if;
          end if;

        else
          -- General default 5-category evaluation for remaining tasks
          v_c1 := 2.0; v_c2 := 2.0; v_c3 := 2.0; v_c4 := 2.0; v_c5 := 2.0;
      end case;

      v_task_score := v_c1 + v_c2 + v_c3 + v_c4 + v_c5;
      v_feedback := 'Evaluated successfully.';
    end if;

    v_total_score := v_total_score + round(v_task_score, 2);

    v_task_details := v_task_details || jsonb_build_object(
      'task_id', v_task.task_id,
      'score', round(v_task_score, 2),
      'evaluation_details', jsonb_build_object('feedback', v_feedback, 'attempted', v_is_modified)
    );
  end loop;

  -- Insert Submission
  insert into public.submissions (
    participant_id, design_json, total_score, submitted_at, status
  ) values (
    v_participant.id, p_design_json, v_total_score, v_submitted_at, 'submitted'
  ) returning id into v_submission_id;

  -- Insert Task Results
  insert into public.task_results (submission_id, task_id, score, evaluation_details)
  select v_submission_id, (item->>'task_id')::text, (item->>'score')::numeric, item->'evaluation_details'
  from jsonb_array_elements(v_task_details) as item;

  -- Finalize Participant Profile
  update public.participants
  set final_score = v_total_score,
...
grant execute on function public.evaluate_submission_rpc(jsonb, text) to anon, authenticated, service_role;
